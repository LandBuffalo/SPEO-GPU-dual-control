...
int IslandEA::SendFlagFinish()
{
    ...
    for(int i = 0; i < node_info_.GPU_num; i++)
    {
        ...
        MPI_Send(&tmp_flag_finish, 1, MPI_INT, destination, tag, MPI_COMM_WORLD);
    }
...
int IslandEA::Execute()
{
    ...
    Finish();
    if (node_info_.GPU_ID == 0)
      EA_CUDA_->VerifyCorrectness(sub_population_);
...
int IslandEA::PrintResult()
{
    ...
    file<<function_ID<<','<<run_ID<<','<<dim<<','<<best_fitness<<','<<problem_info_.computing_time<<','<<communication_percentage<<','<<total_FEs<<','<<node_info_.node_num\
...
int IslandEA::SendResultToIsland0()
{
    ...
    MPI_Send(msg, 3, MPI_DOUBLE, 1, tag, MPI_COMM_WORLD);
#endif
#ifdef GPU_SINGLE_PRECISION
    MPI_Send(msg, 3, MPI_FLOAT, 1, tag, MPI_COMM_WORLD);
#endif
    delete [] msg;
