...
//---------------------------node info----------------------------------------------------//
    node_info.GPU_num = node_info.node_num / 2;
    if (getParam("-GPU_num", argv, argc))
        node_info.GPU_num = atoi(getParam("-GPU_num", argv, argc));
    node_info.GPU_ID = node_info.node_ID / (node_info.node_num / node_info.GPU_num);

    if(node_info.node_ID % (node_info.node_num / node_info.GPU_num) == 0)
        node_info.task_ID = 0;
    else if(node_info.node_ID % (node_info.node_num / node_info.GPU_num) == 1)
	{
		node_info.task_ID = 1;
...
int main(int argc, char* argv[])
{
    ...
    if (node_info.task_ID == -1)
        ConstructAndExecuteVoidModule(total_functions, total_runs, island_info, problem_info, node_info);
